/* Vina NZ — Class list & class detail */
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:18, maxWidth:1180 }}>
      <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
        <div>
          ...
          <p style={{ fontSize:14, color:'var(--ink-500)', marginTop:4 }}>All {VZ.classes.length} classes in one place — filter, open and export.</p>
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, background:'var(--surface)', border:'1px solid var(--line)', borderRadius:'var(--r-sm)', padding:'0 12px', height:38, width:220 }}>
          ...
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search classes…" style={{ border:'none', background:'transparent', flex:1, outline:'none', fontSize:13.5 }} />
        </div>
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
        {[['all','All classes'],['active','Active'],['soon','Starting soon'],['paused','Paused']].map(([v,l])=>(
...
function ClassDetail({ classId, go, notify }){
  ...
  const info = [
    { icon:'teachers', label:'Teacher', value:t.name.replace('Thầy ','').replace('Cô ','') },
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20, maxWidth:1100 }}>
      ...
      <div className="card" style={{ padding:0, overflow:'hidden' }}>
        ...
        <div style={{ padding:'22px 24px', display:'flex', alignItems:'flex-start', gap:20, flexWrap:'wrap' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:9, width:230 }}>
            <button className="btn btn-zalo" onClick={()=>notify('Opening Zalo group: '+c.name,'zalo')}><Icon name="zalo" size={17} />Open Zalo group</button>
            <button className="btn btn-primary" onClick={()=>setExportOpen(true)}><Icon name="printer" size={17} />Export timetable</button>
            <button className="btn btn-ghost" onClick={()=>notify('Edit class','edit')}><Icon name="edit" size={16} />Edit class</button>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:2, padding:'9px 11px', background:'var(--zalo-bg)', borderRadius:'var(--r-sm)' }}>
              <Icon name="link" size={15} style={{ color:'var(--zalo)' }} />
              <span style={{ fontSize:12.5, color:'var(--zalo)', fontWeight:600, flex:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{c.zalo}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <Modal open={exportOpen} onClose={()=>setExportOpen(false)} width={620} title="Export timetable" sub={'Share this timetable to the '+c.name+' Zalo group'}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {bySession[sess].map(m=>(
              <div key={m.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 12px', border:'1px solid var(--line)', borderRadius:'var(--r-md)' }}>
                ...
                {m.to!=='class' && <span className="tag" style={{ background:'var(--p-50)', color:'var(--brand)' }}>Individual</span>}
    ...
    <Modal open={!!material} onClose={onClose} width={460} title="Send material" sub={material.title}>
      ...
      {mode==='individual' && (
        <div style={{ marginTop:16, maxHeight:230, overflowY:'auto', display:'flex', flexDirection:'column', gap:4 }}>
          {roster.map(s=>(
            <label key={s.id} style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 8px', borderRadius:'var(--r-sm)', cursor:'pointer', background:picked.includes(s.id)?'var(--p-50)':'transparent' }}>
              <input type="checkbox" checked={picked.includes(s.id)} onChange={()=>toggle(s.id)} style={{ width:17, height:17, accentColor:'var(--brand)' }} />
...
Object.assign(window, { ClassList, ClassDetail, ClassMaterials, SendMaterialSheet });
