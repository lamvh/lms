/* Vina NZ — Prototype harness: tooling bar + scaling stage + tweaks */
const { useState, useRef, useLayoutEffect, useEffect } = React;

const TWEAK_DEFAULTS = /*EDITMODE-BEGIN*/{
  "theme": ["#15406B","#123A60","#0E2C4B"],
  "accent": "#2EA37A",
  "radius": 1,
...
function App(){
  ...
  const theme = Array.isArray(t.theme) ? t.theme : ['#15406B','#123A60','#0E2C4B'];
  const rootStyle = {
    '--p-700':theme[0], '--p-800':theme[1], '--p-900':theme[2],
    '--brand':theme[0], '--brand-strong':theme[1], '--accent':t.accent,
    '--radius-scale':t.radius,
    '--font-head':`'${t.headFont}', 'Public Sans', system-ui, sans-serif`,
  };

  const TABS = [['login','Login','lock'],['admin','Admin · desktop','dashboard'],['student','Student · mobile','user'],['wireframe','Wireframe','grid']];

  return (
    <div style={{ ...rootStyle, height:'100vh', display:'flex', flexDirection:'column', background:'#1a2330' }}>
      {/* Tooling bar */}
      <header style={{ flexShrink:0, height:54, background:'#141d28', borderBottom:'1px solid #28323f',
        display:'flex', alignItems:'center', gap:18, padding:'0 18px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <Logo size={26} light textSize={15} />
          <span style={{ color:'#5d6b7a', fontSize:12, fontWeight:600, borderLeft:'1px solid #2c3744', paddingLeft:12, fontFamily:'var(--font-head)' }}>LMS Prototype</span>
        </div>
        <div style={{ flex:1 }}></div>
        <div style={{ display:'inline-flex', background:'#1f2a36', borderRadius:10, padding:4, gap:3 }}>
          {TABS.map(([id,label,icon])=>{
            ...
            return (
              <button key={id} onClick={()=>setScreen(id)} style={{
                ...
                background: on?'#2a6298':'transparent', color: on?'#fff':'#8b97a4', transition:'all .15s' }}>
        ...
        <div style={{ flex:1 }}></div>
        <span style={{ color:'#5d6b7a', fontSize:12, display:'flex', alignItems:'center', gap:6 }}>
          <Icon name="sparkle" size={14} style={{ color:'#7b8896' }} />Tweaks: theme · layout</span>
      </header>

      {/* Stage */}
      {screen==='login' && (
        <Stage w={1180} h={720} key="login">
          <ChromeWindow width={1180} height={720} url="app.vinanz.school/login" tabs={[{title:'Vina NZ — Sign in'}]}>
            <LoginScreen onSignIn={(role)=> setScreen(role==='student'?'student':'admin')} />
          </ChromeWindow>
        </Stage>
      )}
      {screen==='admin' && (
        <Stage w={1320} h={840} key="admin">
          <ChromeWindow width={1320} height={840} url="app.vinanz.school/dashboard" tabs={[{title:'Vina NZ — Dashboard'}]}>
            <AdminApp onSignOut={()=>setScreen('login')} dashVariant={dashVariant} setDashVariant={setDashVariant} znsOn={znsOn} setZnsOn={setZnsOn} />
          </ChromeWindow>
        </Stage>
      )}
      {screen==='student' && (
        <Stage w={402} h={860} key="student">
          <IOSDevice width={402} height={860}>
            <StudentApp onSignOut={()=>setScreen('login')} />
          </IOSDevice>
        </Stage>
      )}
      {screen==='wireframe' && (
        <Stage w={1180} h={760} key="wire">
          <div style={{ width:1180, height:760, borderRadius:12, overflow:'hidden', boxShadow:'0 24px 70px rgba(0,0,0,.4)', border:'1px solid #cdd5dd', display:'flex', flexDirection:'column', background:'#dfe4e9' }}>
            <div style={{ height:34, background:'#cdd5dd', display:'flex', alignItems:'center', gap:7, padding:'0 14px' }}>
              ...
              <span style={{ width:11, height:11, borderRadius:99, background:'#aab4bf' }}></span>
              <span style={{ width:11, height:11, borderRadius:99, background:'#aab4bf' }}></span>
              <span style={{ marginLeft:12, fontFamily:'ui-monospace,Menlo,monospace', fontSize:11.5, color:'#6b7884' }}>wireframe — admin dashboard structure</span>
            </div>
      ...
      <TweaksPanel>
        ...
        <TweakColor label="Theme" value={t.theme} onChange={v=>setTweak('theme',v)}
          options={[['#15406B','#123A60','#0E2C4B'],['#1E3A5F','#16304F','#0C1F36'],['#136B66','#0F5854','#0A3F3C'],['#3A3F8F','#2F3578','#222759']]} />
        <TweakColor label="Accent" value={t.accent} onChange={v=>setTweak('accent',v)}
          options={['#2EA37A','#1E8A8A','#C9821F','#2A6298']} />
        <TweakSection label="Style" />
        <TweakSlider label="Corner radius" value={t.radius} min={0.5} max={1.5} step={0.1} onChange={v=>setTweak('radius',v)} />
        <TweakSelect label="Heading font" value={t.headFont} options={['Plus Jakarta Sans','Manrope','Sora']} onChange={v=>setTweak('headFont',v)} />
