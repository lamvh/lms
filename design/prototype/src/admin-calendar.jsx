/* Vina NZ — Timetable (weekly calendar) + export preview */
...
function CalendarView({ openClass, notify }){
  ...
  const days = [0,1,2,3,4,5]; // Mon–Sat (Sun rarely used)
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:18, maxWidth:1180 }}>
      <div style={{ display:'flex', alignItems:'center', gap:14, flexWrap:'wrap' }}>
        <div>
          ...
          <p style={{ fontSize:14, color:'var(--ink-500)', marginTop:4 }}>All classes on one calendar · week of 9–14 June 2026</p>
        </div>
      ...
      <div style={{ display:'flex', gap:14, flexWrap:'wrap' }}>
        {VZ.teachers.map(t=>(
          <span key={t.id} style={{ display:'flex', alignItems:'center', gap:7, fontSize:12.5, color:'var(--ink-500)' }}>
            <span style={{ width:10, height:10, borderRadius:3, background:t.color }}></span>{t.name.replace('Thầy ','').replace('Cô ','')}</span>
        ))}
      </div>
      ...
      <div className="card" style={{ padding:'0', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', position:'relative', height:(DAY_END-DAY_START)*HOUR_H }}>
          ...
          {days.map(d=>(
            <div key={d} style={{ flex:1, position:'relative', borderLeft:'1px solid var(--line-soft)', background: d===VZ.todayIndex?'rgba(42,98,152,.03)':'transparent' }}>
      ...
      <Modal open={exportOpen} onClose={()=>setExportOpen(false)} width={640} title="Export timetable" sub="Clean layout ready to share to a Zalo group">
...
/* Shareable / printable export preview (whole centre or single class) */
    ...
    <div>
      ...
      <div style={{ borderRadius:'var(--r-lg)', overflow:'hidden', border:'1px solid var(--line)', boxShadow:'var(--sh-card)' }}>
        <div style={{ background:'linear-gradient(120deg, var(--p-700), var(--p-900))', padding:'18px 22px', color:'#fff', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <div style={{ textAlign:'right', fontSize:11.5, color:'var(--p-200)' }}>Vina NZ Tutoring Centre<br/>Auckland, NZ</div>
        </div>
        ...
        <div style={{ background:'var(--surface-2)', padding:'10px 22px', fontSize:11.5, color:'var(--ink-400)', textAlign:'center', borderTop:'1px solid var(--line-soft)' }}>
          Generated by Vina NZ · vinanz.school
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:10, marginTop:18 }}>
        ...
        <button className="btn btn-ghost" style={{ flex:1 }} onClick={()=>notify('Timetable downloaded (PDF)','download')}><Icon name="printer" size={16} />Download PDF</button>
        <button className="btn btn-zalo" style={{ flex:1 }} onClick={()=>{notify('Posted to Zalo group','zalo'); onClose&&onClose();}}><Icon name="zalo" size={16} />Post to Zalo</button>
      </div>
    </div>
  );
}
