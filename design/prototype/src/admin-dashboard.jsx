/* Vina NZ — Admin dashboard (3 layout variants) */
  ...
  return (
    <div>
      ...
      {!compact && <p style={{ fontSize:15, color:'var(--ink-500)', marginTop:7 }}>
        You have <b style={{ color:'var(--ink-800)' }}>{todaySessions().length} classes</b> scheduled today across 3 rooms.</p>}
    </div>
  );
}
...
function StatCards(){
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16 }}>
      <Stat icon="classes"  tone="brand" value={s.classes}  label="Active classes" sub="+2 this term" />
...
function TodaySchedule({ openClass, dense }){
  ...
  const nowMin = 17*60+10; // pretend "now" ~ 5:10pm
  return (
    <div className="card" style={{ padding:'20px 22px', display:'flex', flexDirection:'column', minHeight:0 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <span className="tag">{sessions.length} classes</span>
        </div>
        ...
        {sessions.map((s,i)=>{
          ...
          return (
            <div key={i} onClick={()=>openClass(s.class.id)} style={{
              ...
              background: isNow?'var(--p-50)':'transparent',
            }} onMouseEnter={e=>e.currentTarget.style.background=isNow?'var(--p-100)':'var(--surface-2)'}
               onMouseLeave={e=>e.currentTarget.style.background=isNow?'var(--p-50)':'transparent'}>
              <div style={{ width:62, textAlign:'right', flexShrink:0 }}>
                <div style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:14, color: isNow?'var(--brand)':'var(--ink-800)' }}>{to12(s.s)}</div>
...
function QuickActions({ go, notify, znsOn }){
  const acts = [
    ...
    { icon:'zalo', label:'Message a group', tone:'zalo', fn:()=>znsOn?go('zalo'):notify('Zalo add-on is off','zalo') },
  ];
  const toneC = { brand:['var(--p-50)','var(--brand)'], green:['var(--a-50)','var(--a-600)'], plum:['#F2E9F1','#8A5A86'], zalo:['var(--zalo-bg)','var(--zalo)'] };
  return (
...
/* ---- Class list table (used in dashboard + standalone) ---- */
  ...
  return (
    <div className="card" style={{ overflow:'hidden' }}>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'16px 20px', borderBottom:'1px solid var(--line)', flexWrap:'wrap' }}>
        <h3 style={{ fontSize:16, fontWeight:700, marginRight:4 }}>All classes</h3>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13.5, minWidth:680 }}>
          <thead>
            <tr style={{ textAlign:'left', color:'var(--ink-400)', fontSize:11.5, textTransform:'uppercase', letterSpacing:'.06em' }}>
              <th style={{ padding:'11px 20px', fontWeight:700 }}>Class</th>
                ...
                <tr key={c.id} style={{ borderTop:'1px solid var(--line-soft)', cursor:'pointer' }}
                  ...
                  <td style={{ padding:'13px 12px' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      ...
                      <span style={{ color:'var(--ink-600)' }}>{t.name.replace('Thầy ','').replace('Cô ','')}</span>
                    </div>
                  </td>
                  ...
                  <td style={{ padding:'13px 20px' }}>
                    <div style={{ display:'flex', gap:6, justifyContent:'flex-end' }}>
                      <Tip label="Open Zalo group"><button className="btn btn-icon btn-zalo btn-sm" onClick={e=>{e.stopPropagation();notify('Opening Zalo group: '+c.name,'zalo');}}><Icon name="zalo" size={16} /></button></Tip>
...
/* ================= VARIANTS ================= */
function AdminDashboard({ variant='a', setVariant, go, openClass, notify, znsOn }){
  let body;
  if(variant==='b') body = <DashB {...{go,openClass,notify,znsOn}} />;
  else if(variant==='c') body = <DashC {...{go,openClass,notify,znsOn}} />;
  else body = <DashA {...{go,openClass,notify,znsOn}} />;
  return (
    <div>
      {setVariant && (
...
/* A — Balanced overview */
function DashA({ go, openClass, notify, znsOn }){
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:22, maxWidth:1180 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1.7fr 1fr', gap:18, alignItems:'start' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
          <QuickActions go={go} notify={notify} znsOn={znsOn} />
...
/* B — Schedule-first with hero band */
function DashB({ go, openClass, notify, znsOn }){
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20, maxWidth:1180 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:20, flexWrap:'wrap',
        ...
        <div>
          ...
          <p style={{ color:'var(--p-200)', marginTop:6, fontSize:14.5 }}>{todaySessions().length} classes today · all rooms in use after 4pm</p>
        </div>
        <div style={{ display:'flex', gap:26 }}>
          {[['Classes',VZ.stats.classes],['Students',VZ.stats.students],['Teachers',VZ.stats.teachers]].map(([l,n])=>(
            <div key={l} style={{ textAlign:'center' }}>
              <div style={{ fontFamily:'var(--font-head)', fontSize:30, fontWeight:700 }}>{n}</div>
              <div style={{ color:'var(--p-200)', fontSize:12.5 }}>{l}</div>
            </div>
          ))}
          <button className="btn" style={{ background:'#fff', color:'var(--brand)', alignSelf:'center' }} onClick={()=>go('calendar')}>
...
/* C — Class grid focus */
function DashC({ go, openClass, notify, znsOn }){
  const [tab, setTab] = React.useState('all');
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20, maxWidth:1180 }}>
      <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
        ...
        <div style={{ display:'flex', gap:12 }}>
          {[['classes',VZ.stats.classes,'Classes'],['students',VZ.stats.students,'Students'],['calendar',VZ.stats.sessionsToday,'Today']].map(([ic,n,l])=>(
            <div key={l} className="card" style={{ padding:'12px 16px', display:'flex', alignItems:'center', gap:11 }}>
              <span style={{ width:34, height:34, borderRadius:9, background:'var(--p-50)', color:'var(--brand)', display:'flex', alignItems:'center', justifyContent:'center' }}><Icon name={ic} size={18} /></span>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        <h3 style={{ fontSize:16.5, fontWeight:700 }}>Classes</h3>
        <Segmented size="sm" value={tab} onChange={setTab} options={[
          {value:'all',label:'All'},{value:'active',label:'Active'},{value:'soon',label:'Soon'},{value:'paused',label:'Paused'}]} />
        <div style={{ flex:1 }}></div>
  ...
  return (
    <div className="card" style={{ padding:'14px 18px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
        <Icon name="calendar" size={17} style={{ color:'var(--brand)' }} />
        <h3 style={{ fontSize:14.5, fontWeight:700 }}>Today · {VZ.DAYS_FULL[VZ.todayIndex]}</h3>
        <span className="tag">{s.length} classes</span>
      </div>
      <div style={{ display:'flex', gap:12, overflowX:'auto', paddingBottom:4 }}>
        {s.map((x,i)=>(
          <div key={i} onClick={()=>openClass(x.class.id)} style={{ minWidth:184, flexShrink:0, border:'1px solid var(--line)', borderRadius:'var(--r-md)', padding:'12px 13px', cursor:'pointer', borderLeft:'3px solid '+x.class.color }}>
            <div style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:13.5, color:'var(--brand)' }}>{to12(x.s)}–{to12(x.e)}</div>
    ...
    <div className="card" onClick={()=>openClass(c.id)} style={{ padding:0, overflow:'hidden', cursor:'pointer', transition:'box-shadow .15s, transform .1s' }}
      ...
      <div style={{ padding:'16px 17px' }}>
        ...
        <div style={{ display:'flex', gap:8, marginTop:15 }}>
          <button className="btn btn-sm btn-zalo" style={{ flex:1 }} onClick={e=>{e.stopPropagation();notify('Opening Zalo group: '+c.name,'zalo');}}><Icon name="zalo" size={15} />Zalo group</button>
