/* Vina NZ — Admin shell: sidebar + topbar + router */

function AdminApp({ onSignOut, dashVariant, setDashVariant, znsOn, setZnsOn, initialView='dashboard', initialClassId=null }) {
  ...
  function go(v, cid=null){ setView(v); setClassId(cid); }
  ...
  const titles = {
    ...
    students:'Students', zalo:'Zalo Notifications', settings:'Settings',
  };

  let content;
  if (view==='dashboard') content = <AdminDashboard variant={dashVariant} setVariant={setDashVariant} go={go} openClass={openClass} notify={notify} znsOn={znsOn} />;
  ...
  else if (view==='students') content = <PeopleView kind="students" />;
  else if (view==='zalo') content = <ZaloView znsOn={znsOn} setZnsOn={setZnsOn} notify={notify} />;
  ...
  return (
    <div className="app-scope" style={{ height:'100%', display:'flex', background:'var(--bg)', overflow:'hidden', fontFamily:'var(--font-body)' }}>
      ...
      <aside style={{
        ...
        <button className="btn" style={{ background:'rgba(255,255,255,.10)', color:'#fff', justifyContent:'flex-start',
          margin:'0 2px 8px', padding:'10px 12px', border:'1px solid rgba(255,255,255,.12)' }}
        ...
        <div style={{ flex:1, overflowY:'auto', display:'flex', flexDirection:'column', gap:4 }}>
          {NAV.map(grp=>(
            <div key={grp.group} style={{ marginBottom:6 }}>
              ...
              {grp.items.map(it=>(
                <div key={it.id} className={'navlink'+((view===it.id||(it.id==='classes'&&view==='class'))?' is-active':'')}
                  onClick={()=>go(it.id)}>
          ...
          {/* Add-on section */}
          <div style={{ marginTop:4, paddingTop:12, borderTop:'1px solid rgba(255,255,255,.10)' }}>
            <div style={{ fontSize:10.5, fontWeight:700, letterSpacing:'.12em', textTransform:'uppercase',
              color:'var(--p-300)', padding:'4px 12px 6px', opacity:.8, display:'flex', alignItems:'center', gap:6 }}>
              Add-on
            </div>
            <div className={'navlink'+(view==='zalo'?' is-active':'')} onClick={()=>go('zalo')}>
              <Icon name="zalo" size={19} />
              <span style={{ flex:1 }}>Zalo Notifications</span>
              <span style={{ fontSize:10, fontWeight:700, padding:'2px 6px', borderRadius:5,
                background: znsOn?'rgba(46,163,122,.25)':'rgba(255,255,255,.12)',
                color: znsOn?'#9be8cb':'var(--p-200)' }}>{znsOn?'ON':'OFF'}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 8px 4px', borderTop:'1px solid rgba(255,255,255,.10)', marginTop:4 }}>
          <Avatar name="Vũ Lan Phương" short="LP" color="#2EA37A" size={34} />
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ color:'var(--p-300)', fontSize:11.5 }}>Centre admin</div>
          </div>
